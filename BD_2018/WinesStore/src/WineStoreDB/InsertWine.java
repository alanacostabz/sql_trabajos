...
public class InsertWine extends javax.swing.JDialog {
    ...
    private void buildComboViñedo() {
        final String sql1 = "select  vineyard_code, winestore.ref_vineyards.region_code from winestore.ref_vineyards inner join winestore.ref_regions on \n"
                + "winestore.ref_vineyards.region_code = winestore.ref_regions.region_code order by winestore.ref_regions.region_name";
        int ide = 0;
        int aux = 0;
        try {
        ...
        final String sql = "SELECT vineyard_code,vineyard_name FROM winestore.ref_vineyards where region_code = " + aux + " order by vineyard_name";
 ...
 /* else {
    ...
    private void comboCountryFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_comboCountryFocusLost
        ...
        if (comboCountry.getSelectedItem().toString().equals("Seleccionar")) {
            ...
            comboViñedo.addItem(selet);
        } else {
            comboViñedo.removeAllItems();
            viñedo.clear();
            String set = "Seleccionar";
            comboViñedo.addItem(set);
            Integer Pais = (Integer) pais.get(comboCountry.getSelectedItem());
    ...
    private void comboRegionFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_comboRegionFocusLost
        ...
        if (comboRegion.getSelectedItem().toString().equals("Seleccionar")) {
            ...
            Integer reg = (Integer) region.get(comboRegion.getSelectedItem());
            final String sql = String.format("SELECT vineyard_code,vineyard_name FROM winestore.ref_vineyards where region_code = %s order by vineyard_name", reg);
