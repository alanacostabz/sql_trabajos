...
for (int i = 0; i < comboColor.getModel().getSize(); i++) {
            ...
            try {
                if (validacionDatosW(idv) != 0) {
                   ...
                   if (nviñedo == null) {
                    ...
                    if(nregion == null){
                    ...
                    }else  if (nviñedo != null) {
                        String sql5 = String.format("SELECT vineyard_code,vineyard_name FROM winestore.ref_vineyards where region_code = %s order by vineyard_name", Integer.parseInt(nregion));
                        System.out.println(sql5.toString());
                        String one = "";
                        try {
                            comboViñedo.removeAllItems();
                            ...
                            while (rs2.next()) {
                                ...
                                viñedo.put(name, id);
                                if (Integer.parseInt(nviñedo) == id) {
                                    one = name;
                                }
                            }
                            comboViñedo.setSelectedItem(one);
                        } catch (SQLException ex) {
                            System.out.println(ex.getMessage());
                        }
                    }else{
                        ...
                        try {
                            ...
                            comboViñedo.setSelectedItem("Seleccionar");
                        } catch (SQLException ex) {
                            System.out.println(ex.getMessage());
                        }
                    }
        ...
        try {
            ...
            if (aux.isEmpty()) {
                ...
                if (jfecha.getDate() == null) {
                    ...
                    if (input == 0) {
                        String sql = "update winestore.wine_store set color_code = " + co + ", grape_variety_code=" + uv + ", region_code=" + re + ", wine_name='" + n.toUpperCase() + "', price = '" + p + "', "
    ...
    private void comboRegionFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_comboRegionFocusLost
        // TODO add your handling code here:
         Integer re = (Integer) region.get(comboRegion.getSelectedItem());
        final String sql = String.format("SELECT vineyard_code,vineyard_name FROM winestore.ref_vineyards where region_code = %s order by vineyard_name", re);
        System.out.println(sql);
        try {
            comboViñedo.removeAllItems();
            viñedo.clear();
